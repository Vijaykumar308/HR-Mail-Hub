...
const HRDirectoryCreate = () => {
  ...
  useEffect(() => {
    const fetchContact = async () => {
      if (isEditMode) {
        try {
          setIsLoading(true);
          const response = await hrDirectoryService.getHRContactById(id);
          const contact = response.data.hrContact;
          setFormData({
            name: contact.name || '',
            email: contact.email || '',
          ...
          toast.error('Failed to load HR contact details');
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        if (isEditMode) {
          await hrDirectoryService.updateHRContact(id, formData);
        ...
        let errorMessage = isEditMode
          ? 'Failed to update HR contact. Please try again.'
          : 'Failed to create HR contact. Please try again.';
    ...
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex items-center">
        <div className="flex items-center space-x-4">
          ...
          <h2 className="text-2xl font-bold text-gray-900">{isEditMode ? 'Edit HR Contact' : 'Add HR Contact'}</h2>
        </div>
      </div>
